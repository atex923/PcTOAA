...
# - V4.2.x：整合單價分析略過規則、搜尋與字數檢查、項目 0 總工程經費列、
#            右上金額區項目代碼/06 工程管理費/0A3 公共藝術費試算、
#            匯出檔名規則、長檔名顯示修正、工程管理費顯示精度、
#            上半部功能區重排、往後回復編輯、移除匯出 XLS，以及按鈕立體樣式。
# - V4.3.0：視窗最底部新增標語。
# - V4.3.1：組合總表數量顯示三位小數，重新計算時同步右上試算值到對應工項。
# - V4.3.2：右上金額區分原始/重新計算欄，分析前確認是否清除既有資料。
# - V4.3.3：讀入單價統一顯示三位小數，視窗與上方區塊改為可縮放。
# - V4.3.4：012 右欄改為 011×10%，06/0A1/0A2/0A3 右欄無條件取整數。
# - V4.3.5：右上右欄統一顯示三位小數，總經費改為 01+02+03+04+05+06+0A+0B。
# - V4.3.6：右上 0A 工項支援來源/比例計算，新增鎖定回填與總經費讀取下半部複價加總。
# - V4.3.7：鎖定回填新增啟用勾選，右上 0A 動態列擴充到 0AA。
# - V4.3.8：修正 0A 動態列直接讀複價時被鎖定顯示覆蓋的問題。
# - V4.3.9：0A 項下單價讀取第二分頁，並重整鎖定回填與重新計算順序。
...
DEFAULT_LOG_FILENAME = "BudgetAnalyzer_V4_3_9.budget_log.json"
DEFAULT_LOCKED_ITEMS = "012,06,0A1,0A2,0A3,0A4,0A5"
...
class BudgetAnalyzerQT(QMainWindow):
    ...
    def build_ui(self):
        ...
        function_panel.setMinimumWidth(330)
        ...
        top_layout.addWidget(function_panel, stretch=1)
        ...
        self.locked_items_edit = QLineEdit(DEFAULT_LOCKED_ITEMS)
        self.locked_items_check = QCheckBox("啟用鎖定")

        self.file_edit.setMinimumWidth(260)
        self.project_name_edit.setMinimumWidth(300)
        self.project_no_edit.setMinimumWidth(120)
        self.file_type_edit.setMinimumWidth(70)
        self.payment_no_edit.setMinimumWidth(115)
        self.unit_check_edit.setMinimumWidth(70)
        self.hide_level_edit.setMinimumWidth(52)
        self.hide_level_edit.setStyleSheet("background-color: white;")
        self.search_edit.setMinimumWidth(135)
        ...
        self.locked_items_check.toggled.connect(self.update_dynamic_amount_labels)
        self.locked_items_edit.editingFinished.connect(self.update_dynamic_amount_labels)

        for edit in (
            self.file_edit, self.project_name_edit, self.project_no_edit,
            self.file_type_edit, self.payment_no_edit, self.unit_check_edit,
            self.hide_level_edit, self.search_edit, self.locked_items_edit,
        ):
        ...
        fixed_row.addWidget(self.locked_items_check)
        fixed_row.addWidget(small_label("鎖定回填"))
        fixed_row.addWidget(self.locked_items_edit)
        ...
        add_function_row(
            function_button("整理備註", self.clean_combined_remarks, 82),
        ...
        add_function_row(
            function_button("匯出 XLSM", self.export_final_xlsm, 88),
        ...
        self.amount_panel.setMinimumWidth(820)
        ...
        top_layout.addWidget(self.amount_panel, stretch=2)
    ...
    def populate_table(self, table, data):
        ...
        for r, row in enumerate(data):
            for c, col in enumerate(COLUMNS):
                ...
                if table is self.combined_table and col == "數量":
    ...
    def locked_item_codes(self):
        if not self.locked_items_check.isChecked():
            return set()

        return {
            code.strip().upper()
            for code in self.locked_items_edit.text().replace("，", ",").split(",")
            if code.strip()
        }
    ...
    def amount_0a_config(self, code):
        ...
        read_amount_directly = ratio_text == "" or ratio in (None, 0)

        if not source:
            ratio = 1.0
            old_block_state = row["ratio"].blockSignals(True)
            row["ratio"].setText("1")
            row["ratio"].blockSignals(old_block_state)
            read_amount_directly = True

        return source, 1.0 if ratio is None else ratio, read_amount_directly

    def amount_0a_reads_amount_directly(self, code):
        if code not in self.amount_0a_rows:
            return False

        _, _, read_amount_directly = self.amount_0a_config(code)
        return read_amount_directly
    ...
    def dynamic_amount_values(self):
        ...
        for code in RIGHT_TOP_0A_CODES:
            source, ratio, read_amount_directly = self.amount_0a_config(code)

            if read_amount_directly:
                calculated[code] = self.num(self.get_combined_amount_by_item(code)) or 0.0
            elif source:
                calculated[code] = int((self.num(self.get_combined_amount_by_item(source)) or 0.0) * ratio)
            else:
                row = self.get_combined_row_by_item(code)
                calculated[code] = self.num(row.get("單價", "")) if row is not None else 0.0
                calculated[code] = calculated[code] or 0.0
    ...
    def display_amount_snapshot(self, calculated_side=False):
        ...
        if calculated_side:
            ...
            for code in self.locked_item_codes():
                if code in snapshot:
                    snapshot[code] = self.num(self.get_combined_amount_by_item(code)) or 0.0
            return snapshot
    ...
    def collect_state(self):
        return {
            ...
            "locked_items_enabled": self.locked_items_check.isChecked(),
            "locked_items": self.locked_items_edit.text(),
            "amount_0a_settings": {
                code: {
    ...
    def apply_state(self, state):
        ...
        try:
            ...
            self.locked_items_check.setChecked(bool(state.get("locked_items_enabled", False)))
            self.locked_items_edit.setText(state.get("locked_items", DEFAULT_LOCKED_ITEMS))
    ...
    def apply_detail_unit_prices_to_0a_items(self):
        detail_price_by_item = {
            str(row.get("項目", "")).strip().upper(): row.get("單價", "")
            for row in self.detail_data
            if str(row.get("項目", "")).strip().upper().startswith("0A")
            and str(row.get("單價", "")).strip()
        }

        for row in self.combined_data:
            item = str(row.get("項目", "")).strip().upper()

            if item.startswith("0A") and item in detail_price_by_item:
                row["單價"] = detail_price_by_item[item]
    ...
    def sync_dynamic_calculations_to_combined(self, item_to_row):
        values = self.dynamic_amount_values()
        locked_items = self.locked_item_codes()
        target_codes = ("012", "06", "0B", *RIGHT_TOP_0A_CODES)
        changed_cells = []
        changed = 0
        ...
        for item_code in target_codes:
            ...
            unit_price = self.num(row.get("單價", "")) if item_code in locked_items else values["calculated"].get(item_code)

            if unit_price is None:
                continue

            if item_code not in locked_items:
                row["單價"] = self.fmt_3(unit_price)
                changed_cells.append((item_code, "單價"))

            qty = self.num(row.get("數量", ""))
            if qty is not None:
                row["數量"] = self.fmt_3(qty)
                row["複價"] = self.fmt_3(unit_price * qty)
            else:
                row["複價"] = ""
    ...
    def rollup_parent_amounts(self, item_to_row, changed_cells, locked_items):
        rollup_changed = 0
        skipped = 0
    ...
    def calculate_rollup_amounts(self):
        ...
        rollup_changed, rollup_skipped = self.rollup_parent_amounts(item_to_row, changed_cells, locked_items)
