...
DEFAULT_LOG_FILENAME = "BudgetAnalyzer_V4_1_X.budget_log.json"
...
class BudgetAnalyzerQT(QMainWindow):
    def __init__(self):
        ...
        self.color_marks = {}  # (row, col_name) -> QColor，第四分頁計算/刪除顏色
    ...
    def build_ui(self):
        ...
        analysis_row.addWidget(small_label("第四分頁"))
        analysis_row.addWidget(compact_button("階層刪除", self.apply_delete_rules, 92))
        analysis_row.addWidget(compact_button("計算複價", self.calculate_leaf_amounts, 86))
        analysis_row.addWidget(compact_button("計算複價2", self.calculate_rollup_amounts, 92))
        ...
        analysis_row.addWidget(compact_button("刪除備註", self.clean_combined_remarks, 82))
        ...
        io_row = QHBoxLayout()
        io_row.setSpacing(4)
        io_row.addWidget(small_label("第五分頁"))
        ...
        # 雙欄列：左邊抓第四分頁複價，右邊依 01 發包工程費複價試算。
        dual_rows = [
        ...
        self.tabs.addTab(self.combined_table, "詳細+單價組合")
        self.tabs.addTab(self.final_table, "輸出資料")
    ...
    def hide_combined_below_level(self):
        if not self.combined_data:
            self.msg_warn("提醒", "第四分頁尚無資料，請先分析。")
            return
        ...
        self.status_label.setText(f"第四分頁已隱藏第 {level + 1} 層以下資料｜顯示 {len(data)} 筆｜隱藏 {len(self.combined_data) - len(data)} 筆")

    def show_all_combined(self):
        if self.combined_data:
            self.push_edit_history()
        self.refresh_combined_and_final(self.combined_data)
        self.status_label.setText(f"第四分頁已恢復全部顯示｜共 {len(self.combined_data)} 筆")
    ...
    def fill_final_fixed_fields(self):
        ...
        if not self.final_data:
            self.msg_warn("提醒", "第五分頁目前沒有資料可填入。")
            return
        ...
        self.populate_table(self.final_table, self.final_data)
        self.status_label.setText(
            f"已將檔別、動支單號、單位檢核末碼填入第五分頁｜共 {len(self.final_data)} 筆"
        )
        self.save_edit_log(show_message=False)
        self.msg_info("完成", f"已填入第五分頁固定欄位\n共 {len(self.final_data)} 筆")
