...
# - V4.3.14：右上 0A 最右欄依來源/比例規則計算，並調整上方欄框高度。
# - V4.3.15：修正隱藏階層後重新計算會以畫面列號覆蓋錯誤資料列，導致 0A 單價/複價消失。
# - V4.3.16：上半部三個區塊改為可隨視窗比例縮放，避免低解析度視窗遮蔽。
# - V4.2：依指定調整目前正式版號。
# - V4.3.3：重新定義整理備註規則；第四分頁新增欄位清除模式與右鍵雙擊單格編輯。
# - V4.3.3-shift-del：欄位清除改為 Shift+左鍵單擊，並支援 Delete 清除目前選取的單格或多格。
# - V4.3.3-toggle-top：上下半部中間新增 ▲/▼ 按鈕，可暫時隱藏或重新顯示上半部功能區。
# - V4.4：上半部隱藏/顯示按鈕文字固定為「上半部隱藏/顯示開關」，避免版面跳動。
# - V4.4.1：第四分頁新增整理備註2，將備註欄內英文字開頭且含數字的英文數字字串標為藍色。
# - V4.3.3-fix2：修正事件過濾器直接讀取非滑鼠事件造成閃退；保留左鍵三擊清除與右鍵雙擊編輯。
# - V4.3.3-fix：強化左鍵三擊清除判斷，支援雙擊事件銜接第三下，放寬同格點擊間隔。
# - V4.3.1：正式版；第三分頁數量顯示三位小數，第四分頁複價先算到四位，再依第三位四捨五入至二位。
# - V4.3.2：字數檢查上限改 32 字，新增尋找超長文字；第四分頁最後一層工項粉綠底；0B/0B1 刪除與重新計算規則。
# - V4.3.1-xlsx：上半部匯出按鈕由 XLSM 改為 XLSX，匯出檔案副檔名改為 .xlsx。
# - V4.3.b-r4：第四分頁複價先計算至小數第 4 位，再依第 3 位四捨五入到小數第 2 位。
# - V4.3.b-fast：延遲載入 pandas/openpyxl/xlrd，減少啟動時間與初始記憶體。
# - V4.3.b-fix：第四分頁組合總表「複價」先計算到小數後第三位，再四捨五入到小數後第二位。
# - V4.3.b：第四分頁組合總表「複價」計算統一四捨五入到小數後第二位。
# - V4.2.a：第三分頁「預算單價分析表」數量欄讀入與顯示統一到小數後第三位。
...
DEFAULT_LOG_FILENAME = "BudgetAnalyzer_V4_4_1.budget_log.json"
...
REMARK2_PATTERN = re.compile(r"(?<![A-Za-z0-9])([A-Za-z](?=[A-Za-z0-9]*\d)[A-Za-z0-9]+)(?![A-Za-z0-9])")
...
class BudgetAnalyzerQT(QMainWindow):
    ...
    def build_ui(self):
        ...
        add_function_row(
            ...
            function_button("整理備註2", self.clean_combined_remarks2, 88),
    ...
    def clean_combined_remarks2(self):
        if not self.combined_data:
            self.msg_warn("提醒", "組合總表尚無資料，請先分析。")
            return
        ...
        self.status_label.setText(
            f"整理備註2完成｜備註欄藍字標示 {matched_rows} 列｜{matched_words} 個字串"
        )
        self.msg_info(
            "完成",
            f"整理備註2完成\n"
            f"檢查欄位：第四分頁備註\n"
            f"符合字串：{matched_words} 個\n"
            f"影響列數：{matched_rows} 列"
        )
