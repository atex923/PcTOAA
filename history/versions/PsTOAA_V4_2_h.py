...
#   1. 組和總表修正為組合總表。
# - V4.2.a 單價分析讀取調整：
#   1. B欄讀到「人工：」時略過人工區段，直到A欄再次有資料才恢復判讀與項目編碼。
# - V4.2.b 單價分析讀取修正：
#   1. B欄讀到「人工：                    機具：」這類人工/機具標記時略過。
# - V4.2.c 單價分析讀取修正：
#   1. B欄讀到「工作項目:」這類標記時略過該列，下一列繼續判讀。
# - V4.2.d 新增：
#   1. 目前分頁表格搜尋功能。
# - V4.2.e 新增：
#   1. 檢查「項目名稱」「備註」超過 25 個文字並標示米橘色底。
# - V4.2.f 新增：
#   1. 分析時於組合總表與輸出總表第一列插入總工程經費列。
# - V4.2.g 調整：
#   1. 總工程經費列項目改為 0，重新計算時彙總第 2 層工項複價至項目 0。
# - V4.2.h 右上金額區調整：
#   1. 費用名稱補上項目代碼，並新增 0A3 公共藝術費三欄資料。
...
DEFAULT_LOG_FILENAME = "BudgetAnalyzer_V4_2_h.budget_log.json"
...
class BudgetAnalyzerQT(QMainWindow):
    ...
    def build_ui(self):
        ...
        file_row.addWidget(compact_button("檢查字數", self.check_text_lengths, 82))
        ...
        self.amount_panel.setMinimumHeight(198)
        for label in (
        ...
        dual_rows = [
            ...
            (6, self.amount_art_name_label, self.amount_art_summary_value_label, self.amount_art_calc_name_label, self.amount_art_combined_value_label),
        ]
        ...
        amount_panel_layout.addWidget(self.amount_total_name_label, 7, 0)
        amount_panel_layout.addWidget(self.amount_total_value_label, 7, 1, 1, 3)
    ...
    def update_dynamic_amount_labels(self):
        ...
        - 012 承包商利潤及工程保險費：左欄項目 012 複價，右欄 01 × 10%
        - 0A1 空氣污染防制費：左欄項目 0A1 複價，右欄 01 × 0.5%
        - 0A2 二級品管抽驗費：左欄項目 0A2 複價，右欄 01 × 0.1%
        - 0A3 公共藝術費：左欄預算總表 0A3 複價，右欄組合總表 0A3 複價
        ...
        label_values = (
            ...
            (self.amount_art_combined_value_label, amounts["0A3"]),
