...
DEFAULT_LOG_FILENAME = "BudgetAnalyzer_V4_3_8.budget_log.json"
...
class BudgetAnalyzerQT(QMainWindow):
    ...
    def display_amount_snapshot(self, calculated_side=False):
        ...
        if calculated_side:
            ...
            for code in self.locked_item_codes():
                if code in snapshot:
                    if code in RIGHT_TOP_0A_CODES and self.amount_0a_reads_amount_directly(code):
                        continue
                    row = self.get_combined_row_by_item(code)
                    snapshot[code] = self.num(row.get("單價", "")) if row is not None else snapshot[code]
                    snapshot[code] = snapshot[code] or 0.0
            return snapshot
    ...
    def calculate_rollup_amounts(self):
        ...
        row_0a = item_to_row.get("0A")
        if row_0a is not None:
            children_0a = self.rollup_children("0A", item_to_row)
            total_0a = 0.0
            found_0a = False

            for child in children_0a:
                amount = self.num(item_to_row[child].get("複價", ""))
            ...
            if found_0a:
                row_0a["單價"] = self.fmt_3(total_0a)
                qty_0a = self.num(row_0a.get("數量", ""))
                row_0a["複價"] = self.fmt_3(total_0a * qty_0a) if qty_0a is not None else self.fmt_3(total_0a)
                changed_cells.append(("0A", "單價"))
                changed_cells.append(("0A", "複價"))
